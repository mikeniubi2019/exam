...
layui.use(['jquery','table', 'layer', 'admin'], function () {
  ...
  var questionTable = table.render({
    elem: '#questionTable'
    ,url:'/question/questionSearch?pageId='+pageId
    ,toolbar: '#questionToolbar' //开启头部工具栏，并为其绑定左侧模板
    ,defaultToolbar: ['filter', 'exports', 'print']
    ,title: '题目列表'
    ,cols: [[
      {field:'isSingle', title:'单/多选', edit: 'text'}
  ...
  table.on('toolbar(uestionTableFilter)', function(obj){
    ...
    switch(obj.event){
      case 'addOneQuestion':
        $.ajax({
          url:'/question/addOneBlankQuestion?pageId='+pageId,
          type:'get',
  ...
  table.on('edit(uestionTableFilter)', function(obj){
    ...
    $.ajax({
      url:'/question/updateOneQuestion?id='+data.id+'&'+field+'='+value,
      type:'post',
      success:function(data){
        if (data.code!=100){
          layer.msg("更新失败");
